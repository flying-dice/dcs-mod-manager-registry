...
const githubIntegrationSchema = z.object({
    admins: z.array(z.string()).describe("Integration admins, these are the users that can generate tokens to trigger the integration"),
...
const indexDataSchema = z.object({
    ...
    integration: z.union([githubIntegrationSchema]).optional().describe("The integration of the mod, this is used to automatically update the mod")
})
...
const registryIndexItemDtoSchema = indexDtoSchema.omit({ homepage: true, license: true })
...
mods.forEach(modFolder => {
    ...
    writeJsonFile(join(__dirname, "dist", modFolder.name, "index.json"), indexDto)
    writeJsonFile(join(__dirname, "dist", modFolder.name, "latest.json"), latestDto)
})

writeJsonFile(join(__dirname, "dist", "index.json"), indexJson)
writeJsonFile(join(__dirname, "functions", "integrations", "integrations.json"), _(indexJson).keyBy("id").mapValues(it => (it.integration)).value())

const spec = new OpenApiBuilder(baseSchema)

spec.addSchema("RegistryIndex", generateSchema(z.array(registryIndexItemDtoSchema)))
